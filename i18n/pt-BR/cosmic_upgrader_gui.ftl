...
clamav-clean = Verificação concluída: { $scanned } arquivos checados, nada encontrado.
clamav-failed = Não foi possível executar a verificação: { $message }
clamav-infected = Verificação concluída: { $infected } arquivo(s) infectado(s).
clamav-options = Opções de verificação
clamav-scan = Verificar após a atualização do banco de dados de vírus
clamav-scan-description = O ClamAV está instalado. O topgrade mantém o banco de dados em dia; isto verifica com o novo banco assim que ele muda.
clamav-scanning = Banco de dados de vírus mudou — verificando…
clamav-target = O que verificar
nav-welcome = Boas-vindas
notify-errors = Avisar quando uma atualização falhar
notify-errors-description = Falhas são avisadas mesmo com as outras notificações desligadas, a menos que isto também seja desligado.
notify-title-available = Há atualizações disponíveis
notify-title-installed = Atualizações instaladas
notify-upgrades = Avisar sobre atualizações
notify-upgrades-available = Você saberá o que está disponível para instalar.
notify-upgrades-installed = Você saberá o que foi instalado.
welcome-automatic-heading = Instalação de atualizações
welcome-body = Algumas escolhas que vale a pena fazer agora. Todas ficam nas Configurações depois, e nada aqui é permanente.
welcome-clamav = Verificação de vírus
welcome-finish = Pronto
welcome-heading = Configurar as atualizações
welcome-notifications = Notificações
welcome-root-warning = A instalação sem supervisão precisa de permissões de administrador, então a execução agendada é instalada como um serviço de sistema rodando como root. Nada mais neste aplicativo roda como root.

## Package sources

nav-sources = Fontes de pacotes
sources-add-apt = Adicionar fonte APT
sources-add-flatpak = Adicionar remoto Flatpak
sources-add-heading = Adicionar uma fonte
sources-apt-hint = Uma fonte APT é escrita em /etc/apt/sources.list.d e exige permissões de administrador.
sources-changing = Aplicando…
sources-description = Os repositórios de onde seus gerenciadores de pacotes puxam. O topgrade atualiza o que está instalado; estes decidem o que está disponível.
sources-disable-note = Fontes do APT e do dnf são desativadas em vez de excluídas, para que a mudança possa ser desfeita à mão.
sources-disabled = Desativada
sources-enabled = Ativada
sources-flatpak-hint = Um remoto Flatpak é adicionado só para você e não precisa de senha. Aponte para uma URL .flatpakrepo.
sources-heading = De onde vêm os pacotes
sources-name-placeholder = Nome
sources-none = Nenhuma fonte de pacotes encontrada.
sources-privileged = Alterar isto exige permissões de administrador.
sources-reload = Recarregar
sources-remove = Remover
sources-suite-placeholder = Suíte (ex.: stable)
sources-url-placeholder = URL
show-tray-icon-description = Traz a janela para a frente, inicia uma atualização sem abri-la e sai. Não pode ocultar a janela: o Wayland não oferece como desfazer isso.
welcome-show-again = Tela de configuração
welcome-show-again-description = Voltar à primeira tela e às suas escolhas.

releases-restart-failed = Não foi possível iniciar a nova versão: { $message }
releases-restart-unknown-path = a localização deste aplicativo não é conhecida
releases-restarting = Atualizado — reiniciando na nova versão…

releases-found-dismiss = Estes não
releases-found-new-why = Instalados de um arquivo, então nenhum gerenciador de pacotes tem uma versão mais nova. Acompanhar seus lançamentos?
releases-found-watch = Acompanhar estes
upgrade-releases-with-run = Também atualizar programas instalados de páginas de lançamento
upgrade-releases-with-run-description = O topgrade só os atualiza se foram instalados por uma ferramenta que os acompanha. Um pacote instalado à mão não é coberto por nenhuma, então uma atualização os aplica aqui.
releases-found-new = { $count ->
    [one] 1 programa instalado que mais nada atualizará
   *[other] { $count } programas instalados que mais nada atualizará
}
